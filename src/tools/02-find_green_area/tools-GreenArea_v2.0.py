"""
v2.0
减少了 def extract_green_contour部分return的r来弥补停止时的蓝色像素比绿色外扩，也就是：
                if green_started and is_blue:
                    green_end_radius = r -1
                    break
可调试后自行调整为-2。

===
相较于V1.0，此版本主要目的为debug，增加了输出在原图上描绘边界的功能（具体可参BigCircle.py脚本里的更简明的步骤)


为了使过程更加工程化和模块化，基于既往的汇总脚本../00-old_script/core.py进行拆分，获取到如下提取绿色轮廓的核心代码：
input：path
process：建立了一个class，用来识别绿色边界（核心算法）
...
class GreenContourExtractor:
    ...
    def extract_green_contour(self, center_x, center_y, outer_radius, debug=False):
        ...
        if len(valid_radii) < 3:
            ...
            raise ValueError(f"有效边界点太少({len(valid_radii)})")
        ...
        points = points.reshape((-1, 1, 2))  # reshape 为 cv2.polylines 需要的格式
        cv2.polylines(debug_img, [points], isClosed=False, color=(0, 0, 255), thickness=3)
        cv2.imshow('Boundary Points', debug_img)
        cv2.waitKey(0)  # 等待按键
    ...
    def extract(self, debug=True, save_path=None):
        ...
        return contour_mask, pixel_count

# run
if __name__ == "__main__":

    image_path = "../../../tests/001.png"
    ...
    try:
        ...
        mask, pixel_count = extractor.extract(debug=False, save_path="green_contour_mask.png")
