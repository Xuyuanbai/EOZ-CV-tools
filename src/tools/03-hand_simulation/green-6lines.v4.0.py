...
较1.0 减少了 def scan_radial_distance部分return的r来弥补停止时的蓝色像素比绿色外扩，也就是：
            if green_started and is_blue:
                return r - 1
v3.0 因为这次扫描需要最准确的圆心，所以3.0将霍夫曼拟合方案改为了BigCircle.py的寻找圆心的逻辑
v4.0 取消向下取整int() 改为四舍五入round()
"""
...
class GreenDiameterMeasurer:
    ...
    def scan_radial_distance(self, center_x, center_y, angle_deg, max_radius):
        ...
        for r in range(0, int(max_radius * 0.8)):
            x = round(center_x + r * np.cos(angle_rad))
            y = round(center_y + r * np.sin(angle_rad))
    ...
    def measure_diameters(self, show_image=True):
        ...
        if show_image:
            ...
            for angle1, angle2 in valid_pairs:
                ...
                x1 = round(center_x + dist1 * np.cos(angle1_rad))
                y1 = round(center_y + dist1 * np.sin(angle1_rad))

                x2 = round(center_x + dist2 * np.cos(angle2_rad))
                y2 = round(center_y + dist2 * np.sin(angle2_rad))

                cv2.line(result_image, (x1, y1), (x2, y2), (0, 0, 255), 1)

            # 绘制圆
            cv2.circle(result_image, (center_x, center_y), int(outer_radius), (0, 0, 255), 1)
            cv2.circle(result_image, (center_x, center_y), 1 , (0, 0, 0), -1)

            # 显示图像
            #print(f"图像形状: {result_image.shape}, 数据类型: {result_image.dtype}")
            #cv2.imshow("result", result_image)
            cv2.imwrite("result v4.0.png", result_image)
            cv2.waitKey(0)
